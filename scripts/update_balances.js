...
async function getLastBlockHeightInDatabase(assetType) {
    ...
    logger.info(
        `Last block on db for asset ${assetType.type} is ${blockHeight}`
    );
    return blockHeight;
}
...
async function getLatestTransactionsFromAllBlockchains(rewardRateTypes) {
    ...
    for (var rewardRate of rewardRateTypes) {
        ...
        if (startingBlock == 0) {
            startingBlock =
                (await getLastBlockHeightInDatabase(rewardRate)) + 1;
        }
        if (startingBlock < rewardRate.startingBlock) {
            startingBlock = rewardRate.startingBlock;
        }
        let endingBlock = await getLastBlockHeightInBlockchain(
            rewardRate.chainId
        );
        let chainTransactions = await getTransactionsFromBlockchain(
            rewardRate,
            startingBlock,
            endingBlock
        );
 ...
 * @returns
 */
async function getTransactionsFromBlockchain(
    asset,
    startingBlock,
    endingBlock
) {
    ...
    for (
        ...
        logger.info(
            `Fetching events on chain ${chainId} contract ${contractAddress} from block ${iStart} to block ${iEnd}`
        );
...
async function main() {
    ...
    logger = createLogger(
        ...
        `update_balances_${hre.network.name}`
    );
...
async function getUserEarnedPoints(rewardRateTypes, user) {
    ...
    if (earnedPoints == 0 && hre.network.name != "mainnet") {
        logger.info(

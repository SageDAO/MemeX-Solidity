...
describe("Lottery Contract", function () {
    beforeEach(async () => {
        ...
        lottery = await upgrades.deployProxy(Lottery, [rewards.address, owner.address, mockERC20.address], { kind: 'uups' });
        ...
        await lottery.createNewLottery(1, 1, 10, 0, block.timestamp, block.timestamp + 86400 * 3,
            nft.address, true, 0);
        await lottery.createNewLottery(2, 2, 0, 1, block.timestamp, block.timestamp + 86400 * 3,
            nft.address, true, 0);
        lottery.addPrizes(1, [1, 2], [1, 100]);
        lottery.setSignerAddress(owner.address);

        abiCoder = ethers.utils.defaultAbiCoder;
        leafA = keccak256(abiCoder.encode(["address", "uint256"], [addr1.address, 150]));
    ...
    it("Should update a lottery", async function () {
        await lottery.updateLottery(1, 5, ONE_ETH, block.timestamp, block.timestamp + 86400 * 3,
    ...
    it("Should allow users to buy tickets with points", async function () {
        await lottery.connect(addr1).buyTicketsWithSignedMessage(150, 1, 1, signedMessageA);
        await lottery.connect(addr2).buyTicketsWithSignedMessage(1500, 1, 2, signedMessageB);
    ...
    it("Should allow to claim more points if new rewards are published", async function () {
        ...
        await lottery.connect(addr1).buyTicketsWithSignedMessage(300, 1, 1, signedMessageA);
        expect(await lottery.getLotteryTicketCount(1)).to.equal(2);
    });

    it("Should throw if user doesn't have enough points", async function () {
        await lottery.connect(addr1).buyTicketsWithSignedMessage(150, 1, 15, signedMessageA);
        await expect(lottery.connect(addr1).buyTicketsWithSignedMessage(150, 1, 1, signedMessageA)).to.be.revertedWith("Not enough points");
        await expect(lottery.connect(addr1).buyTickets(1, 1)).to.be.revertedWith("Not enough points");
    });

    it("Should throw if buying more tickets than maxEntries", async function () {
        await lottery.connect(addr1).buyTicketsWithSignedMessage(150, 1, 1, signedMessageA);
        await lottery.setMaxTicketsPerUser(1, 1);
        await expect(lottery.connect(addr1).buyTickets(1, 2)).to.be.revertedWith("Can't buy this amount of tickets");
    });
    ...
    it("Should let user buy 10 lottery tickets", async function () {
        await lottery.connect(addr2).buyTicketsWithSignedMessage(1500, 1, 10, signedMessageB);
    ...
    it("Should not let users buy tickets when lottery sold out", async function () {
        ...
        await lottery.connect(addr2).buyTicketsWithSignedMessage(1500, 1, 1, signedMessageB);
        // should fail on the second entry
        await expect(lottery.connect(addr2).buyTicketsWithSignedMessage(1500, 1, 1, signedMessageB)).to.be.revertedWith("Tickets sold out");
    });
    ...
    it("Should allow refunds on a refundable lottery", async function () {
        ...
        await lottery.connect(addr1).buyTickets(2, 1);
        await lottery.updateLottery(2, 10, 0, block.timestamp, block.timestamp + 86400 * 3,
            nft.address, 0, 0, 3, true); // "force" a lottery completion (status = 3)
        expect(await lottery.connect(addr2).askForRefund(2)).to.have.emit(lottery, "Refunded");
        expect(await lottery.connect(addr1).askForRefund(2)).to.have.emit(lottery, "Refunded");
    });
    ...
    it("Should get the user refundable balance", async function () {
        ...
        await lottery.connect(addr1).buyTickets(2, 1);
        await lottery.connect(addr1).buyTickets(2, 1);
        await lottery.updateLottery(2, 10, TWO_ETH, block.timestamp, block.timestamp + 86400 * 3,
            nft.address, 0, 0, 3, true); // "force" a lottery completion (status = 3)
        expect(await lottery.connect(addr2).getRefundableCoinBalance(2, addr2.address)).to.equal(1);
        expect(await lottery.connect(addr1).getRefundableCoinBalance(2, addr1.address)).to.equal(2);
    });
    ...
    it("Should return the correct # of tickets bought", async function () {
        ...
        await lottery.connect(addr1).buyTickets(2, 1);
        await lottery.connect(addr1).buyTickets(2, 1);
        expect(await lottery.connect(addr2).getTicketCountPerUser(2, addr2.address)).to.equal(1);
        expect(await lottery.connect(addr1).getTicketCountPerUser(2, addr1.address)).to.equal(2);
    });
    ...
    it("Should revert if asking for a refund twice", async function () {
        ...
        await lottery.updateLottery(2, 10, TWO_ETH, block.timestamp, block.timestamp + 86400 * 3,
            nft.address, 0, 0, 3, true); // "force" a lottery completion (status = 3)
        expect(await lottery.connect(addr2).askForRefund(2)).to.have.emit(lottery, "Refunded");
        await expect(lottery.connect(addr2).askForRefund(2)).to.be.revertedWith("Participant has no refundable tickets");
    });
    ...
    it("Should not allow to buy tickets without token balance", async function () {
        ...
        await expect(lottery.connect(addr3).buyTickets(2, 1)).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    });

    it("Should not allow to buy tickets with the wrong lottery id", async function () {
        await expect(lottery.buyTickets(3, 1)).to.be.revertedWith("Lottery is not open");
    });
    ...
    it("Should not allow a second RNG request after response received", async function () {
        ...
        await expect(lottery.requestRandomNumber(2)).to.be.revertedWith("Lottery must be closed");
    });
    ...
    it("Should allow a second RNG request if no response was received", async function () {
        ...
        await ethers.provider.send("evm_mine", []);
        await lottery.requestRandomNumber(2);
        expect(await mockRng.fulfillRequest(2, 1)).to.have.emit(lottery, "LotteryStatusChanged");
    });
    ...
    it("Should not call createNewLottery if not admin", async function () {
        await expect(lottery.connect(addr1).createNewLottery(3, 1, 10, TWO_ETH, block.timestamp, block.timestamp + 86400 * 3,
    ...
    describe("Prize merkle tree", () => {
        beforeEach(async () => {
            abiCoder = ethers.utils.defaultAbiCoder;
            leafA = abiCoder.encode(["uint256", "address", "uint256", "uint256"], [2, addr1.address, 1, 0]);
            leafB = abiCoder.encode(["uint256", "address", "uint256", "uint256"], [2, addr2.address, 2, 1]);
            leafC = abiCoder.encode(["uint256", "address", "uint256", "uint256"], [2, addr1.address, 2, 2]);
        ...
        it("Should claim prize with a merkle proof", async function () {
            await lottery.connect(addr1).buyTickets(2, 1);
            await lottery.connect(addr1).claimPrize(2, addr1.address, 1, 0, prizeProofA);
        ...
        it("Should allow to claim more than one prize", async function () {
            ...
            await lottery.connect(addr1).claimPrize(2, addr1.address, 2, 2, prizeProofC);
        ...
        it("Should throw trying to claim twice", async function () {
            await lottery.connect(addr1).buyTickets(2, 1);
            await lottery.connect(addr1).claimPrize(2, addr1.address, 1, 0, prizeProofA);
            await expect(lottery.connect(addr1).claimPrize(2, addr1.address, 1, 0, prizeProofA)).to.be.revertedWith("Participant already claimed prize");
        });
        ...
        it("Should revert if trying to claim a prize after asking for a refund", async function () {
            ...
            await expect(lottery.connect(addr1).claimPrize(2, addr1.address, 1, 0, prizeProofA)).to.be.revertedWith("Participant has requested a refund");
        });

        it("Should allow refund of non winning tickets after claiming prize", async function () {
            expect(await lottery.connect(addr1).buyTickets(2, 1)).to.have.emit(lottery, "TicketSold");
            expect(await lottery.connect(addr1).buyTickets(2, 1)).to.have.emit(lottery, "TicketSold");
            await lottery.updateLottery(2, 10, TWO_ETH, block.timestamp, block.timestamp + 86400 * 3,
                nft.address, 0, 0, 3, true); // "force" a lottery completion (status = 3)
            await lottery.connect(addr1).claimPrize(2, addr1.address, 1, 0, prizeProofA);
        ...
        it("Should revert if asking for a refund after claiming a prize from a single ticket", async function () {
            ...
            expect(await mockRng.fulfillRequest(2, 1)).to.have.emit(lottery, "ResponseReceived");
            await lottery.connect(addr1).claimPrize(2, addr1.address, 1, 0, prizeProofA);
            await expect(lottery.connect(addr1).askForRefund(2)).to.be.revertedWith("Participant has no refundable tickets");
        });
    });
    ...
    describe("Whitelist", () => {
        ...
        it("Should revert if not whitelisted", async () => {
            await expect(lottery.connect(addr1).buyTicketsWithSignedMessage(150, 1, 1, signedMessageA)).to.be.revertedWith("Not whitelisted");
        });
        ...
        it("Should allow purchase if whitelisted", async () => {
            ...
            await expect(lottery.connect(addr1).buyTicketsWithSignedMessage(150, 1, 1,
                signedMessageA)).to.emit(lottery, "TicketSold");
        });
    });
});

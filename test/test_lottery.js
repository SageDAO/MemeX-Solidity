...
describe("Lottery Contract", function () {
    beforeEach(async () => {
        ...
        await lottery.createNewLottery(1, 5, ethers.utils.parseEther("1"), 10, ethers.utils.parseEther("2"), ethers.utils.parseEther("3"), block.timestamp, block.timestamp + 86400 * 3,
            nft.address, true, 0);
    ...
    it("Should update a lottery", async function () {
        await lottery.updateLottery(1, 5, ethers.utils.parseEther("1"), 10, ethers.utils.parseEther("2"), ethers.utils.parseEther("3"), block.timestamp, block.timestamp + 86400 * 3,
            nft.address, 1, 2, 3, true);
        expect(await lottery.getLotteryCount()).to.equal(1);
        lottery = await lottery.getLotteryInfo(1);
        expect(lottery.status).to.equal(3);
        expect(lottery.vipTicketCostPoints).to.equal(5);
        expect(lottery.vipTicketCostCoins).to.equal(ethers.utils.parseEther("1"));
    ...
    it("Should allow members to buy tickets with points and coins", async function () {
        await lottery.connect(addr1).claimPointsAndBuyTickets(1, 1, 150, hexproof, 1, { value: ethers.utils.parseEther("2") });
        await lottery.connect(addr2).claimPointsAndBuyTickets(1, 2, 1500, hexproofB, 1, { value: ethers.utils.parseEther("4") });
    ...
    it("Should allow non member to buy tickets with coins", async function () {
        await lottery.connect(addr2).buyTickets(1, 1, 2,
    ...
    it("Should allow to claim more points if new rewards are published", async function () {
        ...
        hexproof = tree.getProof(keccak256(leafA)).map(x => buf2hex(x.data));
        hexproofB = tree.getProof(keccak256(leafB)).map(x => buf2hex(x.data));
        await lottery.connect(addr1).claimPointsAndBuyTickets(1, 1, 300, hexproof, 1, { value: ethers.utils.parseEther("2") });
    });

    it("Should throw if user doesn't have enough points", async function () {
        await lottery.connect(addr1).claimPointsAndBuyTickets(1, 15, 150, hexproof, 1, { value: ethers.utils.parseEther("30") });
        await expect(lottery.connect(addr1).claimPointsAndBuyTickets(1, 1, 150, hexproof, 1, { value: ethers.utils.parseEther("2") })).to.be.revertedWith("Not enough points");
        await expect(lottery.connect(addr1).buyTickets(1, 1, 1, { value: ethers.utils.parseEther("2") })).to.be.revertedWith("Not enough points");
    });

    it("Should throw if buying more tickets than maxEntries", async function () {
        await lottery.connect(addr1).claimPointsAndBuyTickets(1, 1, 150, hexproof, 1, { value: ethers.utils.parseEther("2") });
        await lottery.setMaxTicketsPerUser(1, 1);
        await expect(lottery.connect(addr1).buyTickets(1, 2, 1, { value: ethers.utils.parseEther("2") })).to.be.revertedWith("Can't buy this amount of tickets");
    });

    it("Should allow user to buy more tickets on a separate transaction", async function () {
        await lottery.connect(addr2).claimPointsAndBuyTickets(1, 1, 1500, hexproofB, 1, { value: ethers.utils.parseEther("2") });
        await lottery.connect(addr2).claimPointsAndBuyTickets(1, 1, 1500, hexproofB, 1, { value: ethers.utils.parseEther("2") });
    ...
    it("Should let user buy 10 lottery tickets", async function () {
        await lottery.connect(addr2).claimPointsAndBuyTickets(1, 10, 1500, hexproofB, 1, { value: ethers.utils.parseEther("20") });
        expect(await lottery.getParticipantsCount(1)).to.equal(1);
    ...
    it("Should not let users buy tickets when lottery sold out", async function () {
        ...
        await expect(lottery.connect(addr2).claimPointsAndBuyTickets(1, 1, 1500, hexproofB, 1, { value: ethers.utils.parseEther("2") })).to.be.revertedWith("Tickets sold out");
    });
    ...
    it("Should allow withdraw funds from ticket sales", async function () {
        ...
        await lottery.withdraw(addr2.address, ethers.utils.parseEther("2"));
        expect(ethers.BigNumber.from(await ethers.provider.getBalance(addr2.address))).to.equal(addr2Balance.add(ethers.utils.parseEther("2")));
    });
    ...
    it("Should not allow user to buy ticket when lottery is not open", async function () {
        ...
        await expect(lottery.buyTickets(1, 1, 2, { value: ethers.utils.parseEther("3") })).to.be.revertedWith("Lottery is not open");
    });

    it("Should not allow to buy tickets without sending funds", async function () {
        await expect(lottery.connect(addr2).buyTickets(1, 1, 2,
    ...
    it("Should not allow to buy tickets with the wrong lottery id", async function () {
        await expect(lottery.buyTickets(2, 1, 2,
            { value: ethers.utils.parseEther("3") })).to.be.revertedWith("Lottery is not open");
    });
    ...
    it("Should run more than one lottery", async function () {
        ...
        await lottery.createNewLottery(2, 5, ethers.utils.parseEther("1"), 10, ethers.utils.parseEther("2"), ethers.utils.parseEther("3"), block.timestamp, block.timestamp + 86400 * 3,
            nft.address, true, 0);
        lottery.addPrizes(2, [3, 4], [1, 1]);
        await lottery.connect(addr2).claimPointsAndBuyTickets(2, 1, 1500, hexproofB, 1, { value: ethers.utils.parseEther("2") });
        await ethers.provider.send("evm_increaseTime", [86000 * 4]); // long wait, enough to be after the end of the lottery
        await ethers.provider.send("evm_mine", []);
    ...
    it.only("Should allow refund points manually", async function () {
    ...
    describe("Merkle tree", () => {
        ...
        it("Should revert if asking a refund after claiming a prize", async function () {
            await lottery.updateLottery(1, 0, ethers.utils.parseEther("1"), block.timestamp, block.timestamp + 86400 * 3,
                nft.address, 0, 0, 0, true);
            await lottery.connect(addr1).buyTickets(1, 2, false,
                { value: ethers.utils.parseEther("2") });
            await ethers.provider.send("evm_increaseTime", [86000 * 4]); // long wait, enough to be after the end of the lottery
            await ethers.provider.send("evm_mine", []);
            await lottery.requestRandomNumber(1);
            expect(await mockRng.fulfillRequest(1, 1)).to.have.emit(lottery, "ResponseReceived");
            await lottery.connect(addr1).claimPrize(1, addr1.address, 1, hexproof);
            await expect(lottery.connect(addr1).askForRefund(1)).to.be.revertedWith("Can't refund after claiming");
        });
    });

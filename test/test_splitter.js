...
describe("Splitter Contract", function() {
    ...
    it("Should split - FTM", async function() {
        addr2Balance = ethers.BigNumber.from(
            await ethers.provider.getBalance(addr2.address)
        );
        addr3Balance = ethers.BigNumber.from(
            await ethers.provider.getBalance(addr3.address)
        );
        addr4Balance = ethers.BigNumber.from(
        ...
        ).to.equal(addr2Balance.add(100));
        expect(
            ethers.BigNumber.from(
                await ethers.provider.getBalance(addr3.address)
            )
        ).to.equal(addr3Balance.add(450));
        expect(
            ethers.BigNumber.from(
                await ethers.provider.getBalance(addr4.address)
            )
        ).to.equal(addr4Balance.add(450));
    });

    it("Should revert if splitting more than owned balance - ERC20", async function() {
        await expect(
            splitter.splitERC20(1001, mockERC20.address)
        ).to.be.revertedWith("Not enough balance");
    });

    it("Should revert if splitting more than owned balance - FTM", async function() {
        await expect(splitter.split(1001)).to.be.revertedWith(
            "Not enough balance"
        );
    });
});
